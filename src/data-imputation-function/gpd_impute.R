...
gpd_impute<-function(data, m=5, u, run){  #m iis number of imputations, u is the threshold
  data_no_miss<-data[!is.na(data$y),] #data with y not missing
  data_miss<-data[is.na(data$y),] #data with y missing
  ...
  for (i in 1:m){
    ...
    data_imp<-data_no_miss[bootstrap_indices,] #data for imputation
